...
{
//    tq = malloc(sizeof(__omp_task_q_t));
/*
    tq->head = malloc(sizeof(omp_task_q_node_t));
    tq->tail = malloc(sizeof(omp_task_q_node_t));
    tq->head->task = NULL;
    tq->tail->task = NULL;
    tq->head->prev = NULL;
    tq->tail->next = NULL;
*/
//  tq->head = uth_create(NULL, NULL);
  tq->head = co_create(NULL, NULL, NULL, 4096);
  if(tq->head == NULL)
    printf("head = NULL\n");
  //  tq->tail = uth_create(NULL, NULL);
...
/*
inline void __ompc_dec_mod(unsigned int *x)
{
  if(*x == 0)
    *x = __omp_task_q_limit - 1;
  else
    *x = *x - 1;
}
*/
...
/*
inline void __ompc_task_q_dec_tail(omp_task_q_t *tq)
{
  if(tq->tail == 0)
    tq->tail = __omp_task_q_limit - 1;
  else
    tq->tail = tq->tail - 1;
}
*/
