...
extern std::vector<WN *> acc_doloop_side_effects;

BOOL g_bOffloadRegion = FALSE;
...
static WN *expand_acc_rhs_expr_and_cache_doloop_side_effects(gs_t expr, const char *prefix)
{
    WN *wn_tmp;
    WN *result;
...
/******OpenACC Kernel******************************/
/**************************************************/
void expand_acc_start_do_loop (gs_t init_expr, gs_t logical_expr, gs_t incr_expr)
{
    bool valid_for_expr = true;
    gs_code_t code, code1;
    gs_t temp; 
    ...
    FmtAssert (acc_doloop_side_effects.empty(),
               ("Expected DO-loop side-effects to be empty."));
	//////////////////////////////////////////////////////////////////
	/////////////Init statement////////////////////////////
    code = gs_tree_code (init_expr);
    ...
    else 
    {
      ...
      else
      {
        ...
        else
        {
          index = WN_CreateIdname(0,WN_st_idx(lcv));
          WN *rhs;
          rhs = expand_acc_rhs_expr_and_cache_doloop_side_effects(gs_tree_operand(init_expr, 1), "__doloop_init"); 
    ...
    else
    {
        ...
        ub = expand_acc_rhs_expr_and_cache_doloop_side_effects(gs_tree_operand(logical_expr, 1), "__doloop_ub"); 
	...
	///////////////////////////////////////////////////
	///////////////validation the whole for statement////////
    if (valid_for_expr) 
    {
      WGEN_expand_acc_start_do_loop (index, start, end, step);
    }
    else
    {
      char *filename, *dirname; 
...
void expand_acc_end_do_loop (void)
{
    WGEN_expand_acc_end_do_loop();
}

bool ACCIsInKernelRegion = false;
...
{
     WGEN_expand_acc_end_kernel ();
}

extern void WGEN_ACC_Expand_DO (gs_t);

//Loop process in OpenACC
void
...
void
expand_acc_end_forloop (void)
{
     WGEN_expand_acc_end_forloop ();
}
...
void expand_acc_start_parallel (gs_t stmt)
{
  g_bOffloadRegion = TRUE;
  WGEN_expand_acc_start_parallel (stmt);
  WGEN_Expand_Stmt (gs_acc_parallel_body(stmt));
  expand_acc_end_parallel ();
  g_bOffloadRegion = FALSE;
}

void expand_acc_end_parallel ()
{
     WGEN_expand_acc_end_parallel ();
}
...
/*void expand_acc_start_loop (gs_t stmt)
{
  WGEN_expand_acc_start_loop (stmt);
  WGEN_Expand_Stmt (gs_acc_loop_body(stmt));
  expand_acc_end_loop ();
}

void expand_acc_end_loop ()
{
     WGEN_expand_acc_end_loop ();
}*/
...
void expand_acc_start_host_data (gs_t stmt)
{
  WGEN_expand_acc_start_hostdata (stmt);
  WGEN_Expand_Stmt (gs_acc_hostdata_body(stmt));
  expand_acc_end_host_data ();
}

void expand_acc_end_host_data ()
{
     WGEN_expand_acc_end_hostdata ();
}
...
void expand_acc_start_data (gs_t stmt)
{
  WGEN_expand_acc_start_data (stmt);
  WGEN_Expand_Stmt (gs_acc_data_body(stmt));
  expand_acc_end_data ();
}

void expand_acc_end_data()
{
     WGEN_expand_acc_end_data ();
}
...
void expand_acc_start_update (gs_t stmt)
{
  WGEN_expand_acc_start_update (stmt);
  expand_acc_end_update ();
}

void expand_acc_end_update ()
{
     WGEN_expand_acc_end_update ();
}
...
void expand_acc_start_cache (gs_t stmt)
{
  WGEN_expand_acc_start_cache (stmt);
  expand_acc_end_cache ();
}

void expand_acc_end_cache()
{
     WGEN_expand_acc_end_cache ();
}
...
void expand_acc_start_declare(gs_t stmt)
{
  WGEN_expand_acc_start_declare (stmt);
  expand_acc_end_declare ();
}

void expand_acc_end_declare ()
{
     WGEN_expand_acc_end_declare ();
}
...
void expand_acc_start_wait(gs_t stmt)
{
  WGEN_expand_acc_start_wait (stmt);
  expand_acc_end_wait ();
}

void expand_acc_end_wait ()
{
     WGEN_expand_acc_end_wait ();
}
