...
{
  bool modified = false;
  char *fixed_information;

  /* replace '"' with '\"' in the returned string */
  std::stringstream ss;
  std::string string_information(information);

  size_t found = string_information.find("\"");
  while (found != std::string::npos) {
      size_t pos;
      size_t len;
      modified = true;
      ss << string_information.substr(0, found-1);
      ss << "\\\"";
      pos = found+1;
      len = strlen(string_information.data()) - pos;
      string_information = string_information.substr(pos, len);
      found = string_information.find("\"");
  }
  ss << string_information;

  if (modified) {
      fixed_information =
          (char *) malloc(strlen(ss.str().data())+1);
      strcpy(fixed_information, ss.str().data());
  }

   switch (infoId)
   {
   case 1:
      info1 = modified ? fixed_information : information;
      break;
   case 2:
      info2 = modified ? fixed_information : information;
      break;
   case 3:
      info3 = modified ? fixed_information : information;
      break;
   default:
