...
int main()
{
  ...
  #pragma acc parallel copyin(input[0:NI]) \
                       ...
                       vector_length(128) 
  {
    #pragma acc loop gang worker vector reduction(*:product)
	for(i=0; i<NI; i++)
	    product *= input[i];
  }
    gettimeofday(&tim, NULL);
    ...
    {
        error++;
		printf("same_gang_worker_vector + FAILED! product=%d, known_product=%d\n", product, known_product);
    }
    
    printf("same_gang_worker_vector + execution time is :%.2lf: ms\n", end-start);

    if(error == 0)
        printf("same_gang_worker_vector + SUCCESS!\n");
