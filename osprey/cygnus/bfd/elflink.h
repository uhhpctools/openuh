  ...
  for (isym = isymbuf, isymend = isymbuf + extsymcount;
      ...
      if (is_elf_hash_table (hash_table))
	{
	  ...
	  if (isym->st_size != 0
	      && (definition || h->size == 0))
	    {
	      /* Bug 2051: If <abfd> is not WHIRL object symbols, the <old_bfd>
		 is NULL because function _bfd_generic_link_add_one_symbol
		 "only changes definitions for WHIRL object symbols only." */
	      if (h->size != 0 && h->size != isym->st_size && ! size_change_ok
		  && ipa_is_whirl(abfd))
		/* <old_bfd> could be a NULL pointer if the defining section is
		   a dummy section, i.e., bfd_whirl_text_section_ptr, which has
		   no owner.
		   Should we use h->root.u.def.section instead ???
		   (bug#2051)
		*/
		if( old_bfd == NULL ){
		  char* old_src =
		    h->root.u.def.invalid_section_owner
		    ? bfd_archive_filename((bfd*)(h->root.u.def.section))
		    : "previous object";
