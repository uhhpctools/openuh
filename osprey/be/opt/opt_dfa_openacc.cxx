...
typedef struct BB_DATAFLOW_ACC_REGION_INFO
{
	...
	//all of them must be private. They are generated during LNO.
	vector<bool> compiler_gen_acc_tmp; 
...
static BOOL dfa_scan_is_compiler_gen_acc_tmp(ST* st_var)
{
	char* szname = ST_name(st_var);
	char* sz_target = "__acc_sr_tmp_";
	if(strncmp(sz_target, szname, strlen(sz_target)) == 0)
		return TRUE;
	else
		return FALSE;		
}
...
void perform_global_dfa(CFG* cfg)//WDFA_TYPE wdfa_type)
{
	...
	{	
			...
			|| (WN_pragma(WN_first(WN_region_pragmas(wn_region)))==WN_PRAGMA_ACC_PARALLEL_BEGIN)))
		{
			...
			for(INT32 bit_index = 0; bit_index < acc_total_num_vars; bit_index++)
			{
				...
				pRegionInfo->compiler_gen_acc_tmp.push_back(FALSE);				
			}
			...
			while (tree_iter != LAST_POST_ORDER_ITER)
			{
		        ...
		        switch (WN_operator(wn)) 
				{
		            case OPR_LDID:
						...
						if(dfa_scan_is_compiler_gen_acc_tmp(used_var))
							pRegionInfo->compiler_gen_acc_tmp[bitvector_index] = TRUE;
	...
	for(; itor2!=acc_used_data_classify.end(); itor2 ++)
	{
		...
		//ignore the temp variables generated by compiler for acc offload region in the IN
		vector<bool> tmpAccVarMask = dfa_not_operation_bitvector(&pRegionInfo->compiler_gen_acc_tmp);;
		filterIN = dfa_and_operation_bitvector(&filterIN, &tmpAccVarMask);
