                                    ...
                                    INT cur_sec_disp)
{
  ...
  if (Summary->Has_procedure_entry ()) 
  {
	...
	for(i=0; i<size; i++)
	{
		SUMMARY_PROCEDURE* summary_proc = Summary->Get_procedure(i);
		CFG_NODE_INFO* ipa_cfg = Get_cfg_node (0);
		PROJECTED_REGION * ipa_projected_region = Get_projected_region(0);
		REGION_ARRAYS * ipa_region_array = Get_region_array(0);
		
		UINT32 sindex = summary_proc->Get_symbol_index();
		//global symbol
		SUMMARY_SYMBOL* summary_sym = Summary->Get_symbol(0);
		SUMMARY_SYMBOL& sum_symbol = summary_sym[sindex];
		...
		printf("fun name: %s\n", ST_name(st));
		//let begin traverse the SUMMARY_PROCEDURE
		//see the projected_region for each function.		
		INT start_idx = summary_proc->Get_array_section_index();
		INT end_idx = summary_proc->Get_array_section_count() + start_idx;
		INT j;
		for (j = start_idx; j < end_idx; ++ j) 
		{
			// mod region array
			CFG_NODE_INFO* cfg_info = &ipa_cfg[i];
			//only care about the do loop
			...
			for (k = start_region_idx; k < end_region_idx; ++k) 
			{
				//update_region(node, &ipa_regions_array[j]);
				REGION_ARRAYS * r = &ipa_region_array[k];
				INT32 start_pr_idx = r->Get_idx();
  				INT32 count_pr = r->Get_count();
				INT32 end_pr_idx = start_pr_idx + count_pr;
				SUMMARY_SYMBOL* sym_array = &summary_sym[r->Get_sym_id()];  
				ST_IDX st_idx = sym_array->St_idx ();
				printf("def name: %s\n", ST_name(st_idx));
				int kij = 0;
				for (kij=start_pr_idx; kij<end_pr_idx; ++kij) 
				{
					BOOL isMessy = TRUE;
				    PROJECTED_REGION* pr;
				    PROJECTED_REGION* p = &ipa_projected_region[kij];
					isMessy = p->Is_messy_region();
					printf("isMessy:%d.\n", isMessy);
				}
			}
