...
extern int 
putenv_ (char *fchar, int len)
{
    char *envstr;
		/* truncating unused blanks at end of character buffer */
    for (--len;fchar[len]==' ';len--)
	;
    if (!(envstr = (char *) malloc((size_t)(++len)+1)))
	return(-1);
    strncpy( envstr, fchar, len );
    envstr[len] = '\0';
