...
#ifdef KEY
/* Updated RECHAIN_STMTS for GNU 4: "substmt" has already been added
   as sub-statement of some statement. But we may have generated it
   in the current statement_list also, in that case remove it.
   Assumption: if it exists it should be the last stmt in the current
   statement_list. */
#define RECHAIN_STMTS(substmt)                           \
  do {                                                   \
    tree_stmt_iterator i = tsi_last(cur_stmt_list);      \
    if (!tsi_end_p(i)) {                                 \
      tree last_stmt = tsi_stmt(i);                      \
      if (substmt == last_stmt)                          \
        tsi_delink(&i);                                  \
    }                                                    \
  } while (0)
#endif
