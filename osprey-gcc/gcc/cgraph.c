  ...
  if (!node->reachable && node->local.finalized)
    {
      ...
      node->reachable = 1;
#ifdef KEY
      /* What if this DECL has not yet been translated to gs_t? The update
       * in gs_x_1 should handle that case if it ever occurs.
       */
      if (flag_spin_file && gspin_invoked(node->decl))
        gs_set_flag_value (node->decl, GS_DECL_REACHABLE, 1);
#endif
...
cgraph_mark_needed_node (struct cgraph_node *node)
{
  node->needed = 1;
#ifdef KEY
  /* Any ELSE case will be handled in gs_x_1. */
  if (flag_spin_file && gspin_invoked(node->decl))
    gs_set_flag_value (node->decl, GS_DECL_NEEDED, 1);
#endif
